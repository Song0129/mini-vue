...
function genExprsssion(node, context) {
    const { push } = context;
    push(`${node.content}`);
}
...
function parseInterPolation(context) {
    ...
    advanceBy(context, clostDelimiter.length);
    return {
...
function creatVNodeCall(context, tag, props, children) {
...
function transformElement(node, context) {
    if (node.type === 2 /* NodeTypes.ELEMENT */) {
        return () => {
            ...
            node.codegenNode = creatVNodeCall(context, vnodeTag, vnodeProps, vnodeChildren);
        };
    }
}
...
export { creatRenderer, createApp, createVNode as createElementVNode, createTextVNode, effect, getCurrentInstance, h, inject, isProxy, isReactive, isReadonly, isRef, nextTick, provide, proxyRefs, reactive, readonly, ref, registerRuntimeCompiler, renderSlots, shallowReadonly, stop, toDisplayString, unRef };
